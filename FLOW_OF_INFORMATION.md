...
- **Vertical loop (per request):** Architect → Bundle → Executor → QA → feedback.
- **Longitudinal loop (per case):** mutates prompt blocks to reduce vertical loop
  friction for that case over time.

Only the **longitudinal** loop mutates templates. The **vertical** loop does not
rewrite prior context; it appends feedback and retries until stable.

## DSPy Components (How They Connect)
- `MPPAutoAdapter`: DSPy module that runs the vertical loop for a single request.
- `MPPVerticalRefiner`: wrapper around the same vertical loop with explicit
  `build_bundle` / `execute` / `run` entry points.
- `MPPAutoAdapterOptimizer`: DSPy teleprompter that compiles an optimized
  `MPPAutoAdapter` from a single case (longitudinal refinement).

Flow:
1. Start with `MPPAutoAdapter` (vertical runtime behavior).
2. Use `MPPAutoAdapterOptimizer.compile()` with a single case to produce an
   optimized adapter.
3. The compiled adapter still runs the same vertical loop; only the prompt
   blocks (primers/strategy payload) are updated.

## Vertical Refinement (Per Request)
