...
class ProtocolArchitect(dspy.Signature):
    ...
    meta_protocol_version: str = dspy.OutputField(
        desc="MPP spec version (e.g., 1.5.0)."
    )
...
class ProtocolExecutor(dspy.Signature):
    """Execute a self-contained MPP bundle.

    The executor is intentionally "MPP-unaware" at the call boundary: it receives
    the entire bundle as text and must use the derivative protocol spec +
    the task encoded inside the bundle to produce the final response.
    """

    bundle_text: str = dspy.InputField(
        desc="Full MPP message bundle as text (typically JSON)."
    )
    decoded_bundle: Any = dspy.OutputField(
        desc="Task output generated by the ProtocolExecutor (string or JSON)."
    )
...
class QualityAssurance(dspy.Signature):
    ...
    decoded_bundle: str = dspy.InputField(
    ...
    repair_examples: list[str] = dspy.OutputField(
        desc=(
            "Few-shot examples showing the expected output shape; use [] when "
            "verdict is pass or nothing needs repair."
        )
    )
